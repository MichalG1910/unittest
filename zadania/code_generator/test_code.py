...
class TestGetCodeWithDay(unittest.TestCase):
    ...
    def test_get_code_with_day_with_today_date(self, mock_int):
        ...
        actual = get_code_with_day()                            # tak stworzony test zadziała tylko wtedy, kiedy wykonamy
        expected = 'CX-5-WED'                                   # go w środę(expected = 'CX-5-TUE'). Aby był on poprawny 
        self.assertEqual(actual, expected)                      # też w inne dni tyg, musimy zmokować funkcję get_today_name()

    
    @patch('code_generator.get_today_name')                     # zwróć uwagę, jaki jest zapis mokujący. Zawiera się w nim nazwa pliku i metody mokowanej
    @patch('random.randint')
